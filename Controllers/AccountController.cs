...
namespace WPFMedinova.Controllers
{
    public class AccountController : Controller
    {
            ...
            else
            {
                ...
                else if (logObj.Role == "Patient")
                {
                    ...
                    if (data2.Count() > 0)
                    {
                        ...
                        //Create the identity for Doctor
                        identity = new ClaimsIdentity(new[]
                        {
                    new Claim(ClaimTypes.Name, logObj.Username),
        ...
        public IActionResult Appointment()              //after patient registration , appointment need to be booked
        {
        ...
        public async Task<IActionResult> Appointment(AppointmentModel appObj)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    TempData["insert"] = "<script>alert('Appointment Booked Failed!!')</script>";
                }
            }
            else
            {
            ...
            //var data = _dbcontext.Register_Table.Where(m => m.FirstName == TempData["firstname"] && m.LastName == TempData["lastname"] && m.MobileNo== TempData["mobileno"]).FirstOrDefault();
            //return View(data);
            return View();
        }
    }
